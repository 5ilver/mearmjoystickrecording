/*
MeArm.Joystick is dervied from Ben Gray's MeArm (http://www.thingiverse.com/thing:360108)
MeArm.Joystick site : mearm.weebly.com
v1.6.1 by Ted Lien http://www.instructables.com/id/MeArmJoystick-recording/
updated by 5ilver to include pin defs, update comments, and fix position 1 in playback.
 
pins: 
11/10/9/5 : Servo base/left/right/gripper
A0/A1/A2/A3 : Pots (to vcc and ground)
2/4 : Buttons L/R (to ground)
12/13 : Bluetooth rx/tx
*/
#include <Servo.h> 
...
int aOrg[] = {90, 105, 105};  // initial angle for base/left/right arm arm in auto sequence
int demoActions[9][4] = { { 90,  105, 105, 30},    // demo mode by holding right button on boot 
                          { 45, 145,  90,  0},
                          { 90,  55, 165,  0},
                          {135, 145,  90, 30},
                          { 90,  55, 165, 30},
...
int threshL = 200, threshR = 800;
int clawOpenangle = 60, clawCloseangle = 2;
...
void setup() {
  ...
  // hold left button on poweron or reset for recording mode
  ...
  // hold right button on poweron or reset for demo mode
...
void init_Pins(){
  ...
  MIN[0] = 0;           // minimal angle of this servo based on mechanic structure
  ...
  if (!autoPlayMode){
    if (learningMode){
      ...
      if (buttonPreState && buttonPressed) return; // repeated count of the same press
        ...
        buttonPreState = true;
        if (actionIndex + 1 < maxAutoActions){
...
void autoPlay(){
  ...
  while(1){    
    ...
    delay(500/demospeed);
    armfromto(autoAction[actionIndex-1], autoAction[0]);  //back to the first action
    delay(500/demospeed);
  }
}
